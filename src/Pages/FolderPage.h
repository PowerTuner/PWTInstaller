...
namespace PWTI {
    class FolderPage final: public Page {
        Q_OBJECT

    private:
        QLineEdit *installPath = nullptr;

    public:
        explicit FolderPage(const QSharedPointer<installData> &installData);

    private slots:
        void onBrowseBtnClicked();
        void onPathTextChanged(const QString &text) const;
